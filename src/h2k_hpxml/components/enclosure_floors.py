...
def get_floors(h2k_dict, model_data=None):
    ...
    h2k_exp_floors = h2k_exp_floors if isinstance(h2k_exp_floors, list) else [h2k_exp_floors]
    ...
    for floor in h2k_exp_floors:
        ...
        buffered_attached_type = "other non-freezing space" if attached_unit else "outside"

        floor_exterior = (
            buffered_attached_type if floor["@adjacentEnclosedSpace"] == "true" else "outside"
        )
