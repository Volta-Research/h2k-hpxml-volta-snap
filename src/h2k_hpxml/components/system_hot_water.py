...
def get_single_dhw_system(system_dict, sys_id, model_data):
    ...
    if combi_related_hvac_id is not None:
        # H2k does not allow a secondary DHW system when a combo is defined
        ...
        hpxml_water_heating = {
            ...
            "WaterHeaterType": (
                "instantaneous water heater" if combo_tank_volume == 0 else "storage water heater"
            ),
            "Location": tank_location,
...
def get_solar_dhw_system(system_dict, sys_id, model_data):
    ...
    if results != {}:
        ...
        secondary_dhw_energy = float(obj.get_val(results, "Annual,HotWaterDemand,@secondary"))
        solar_dhw_fraction = round(solar_dhw_energy / (solar_dhw_energy + secondary_dhw_energy), 2)
